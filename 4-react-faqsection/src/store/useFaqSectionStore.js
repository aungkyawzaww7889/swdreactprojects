...
const useFaqSectionStore = create((set)=>({
    questions : [
        {
          id: 1,
    ...
    setQuestion: (stateId)=> set((state)=>({
        questions: state.questions.map(el => el.id === stateId ? {...el,isOpen : !el.isOpen} : {...el,isOpen : false})
    }))
}));

export default useFaqSectionStore;